...
data BitBuffer = BitBuffer
  { bufferBytes :: !BS.ByteString  -- ^ Raw byte storage
  , bitPos      :: !Int            -- ^ Write cursor (bits)
  , readPos     :: !Int            -- ^ Read cursor (bits)
  }
  deriving (Show)

-- | Result of a read operation: value plus updated buffer.
data ReadResult a = ReadResult
  { readValue  :: !a          -- ^ The deserialized value
  , readBuffer :: !BitBuffer  -- ^ Buffer with advanced read cursor
  }
  deriving (Show)
