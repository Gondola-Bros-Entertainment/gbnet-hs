...
data DeliveryMode
  ...
  | UnreliableSequenced -- ^ Unreliable but drops out-of-order
  | ReliableUnordered   -- ^ Guaranteed delivery, any order
  | ReliableOrdered     -- ^ Guaranteed delivery, strict order
  | ReliableSequenced   -- ^ Guaranteed delivery, drops out-of-order
...
data ChannelConfig = ChannelConfig
  ...
  , ccBlockOnFull         :: !Bool
  , ccOrderedBufferTimeout :: !Double  -- ^ Milliseconds
...
unreliableConfig = defaultChannelConfig
...
reliableOrderedConfig = defaultChannelConfig
  { ccDeliveryMode = ReliableOrdered
  }

-- | Reliable sequenced channel configuration.
reliableSequencedConfig :: ChannelConfig
reliableSequencedConfig = defaultChannelConfig
...
newChannel channelId config = Channel
  { chConfig              = config
  ...
  where
    ...
    checkRetransmit seqNum msg (acc, c)
      ...
      | cmRetryCount msg > maxRetries =
          ...
          (acc, c { chSendBuffer = Map.delete seqNum (chSendBuffer c)
      ...
      | elapsedMs (cmSendTime msg) now >= rtoMs =
          ...
          let msg' = msg { cmSendTime = now, cmRetryCount = cmRetryCount msg + 1 }
              c' = c { chSendBuffer = Map.insert seqNum msg' (chSendBuffer c)
...
onMessageReceived seqNum payload now ch =
  case ccDeliveryMode (chConfig ch) of
    Unreliable ->
      ch { chReceiveBuffer = chReceiveBuffer ch ++ [payload]
         , chTotalReceived = chTotalReceived ch + 1
         }

    UnreliableSequenced ->
      if sequenceGreaterThan seqNum (chRemoteSequence ch)
        then ch { chReceiveBuffer  = chReceiveBuffer ch ++ [payload]
    ...
    ReliableSequenced ->
      if sequenceGreaterThan seqNum (chRemoteSequence ch) || seqNum == chRemoteSequence ch
        ...
        else ch { chPendingAck   = seqNum : chPendingAck ch
...
deliverOrdered payload ch =
  let ch' = ch { chReceiveBuffer   = chReceiveBuffer ch ++ [payload]
...
flushOrderedBuffer ch =
  case Map.lookup (chOrderedExpected ch) (chOrderedReceiveBuffer ch) of
    Nothing -> ch
    Just (payload, _) ->
      let ch' = ch { chOrderedReceiveBuffer = Map.delete (chOrderedExpected ch) (chOrderedReceiveBuffer ch)
