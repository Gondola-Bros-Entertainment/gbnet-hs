...
- `src/GBNet/Serialize/TH.hs` - Template Haskell deriveNetworkSerialize for records/enums
- `src/GBNet/Packet.hs` - PacketType enum, PacketHeader (68-bit wire format), serialization instances
- `test/Main.hs` - 60 round-trip tests covering all types and TH derive
- `.github/workflows/ci.yml` - GitHub Actions CI (build, test, -Wall -Werror, HLint)
...
cabal build --ghc-options="-Wall -Werror"    # Warnings as errors
```
